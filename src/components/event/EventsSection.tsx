...
interface Event {
  ...
  capacity: number;
  registered: number;
  imageColor: string; // placeholder تا تصویر واقعی بیاد
  tags: string[];
}
...
export default function EventsSection() {
  ...
  return (
    <section dir="rtl" className="w-full bg-white dark:bg-[#0B0F19] transition-colors duration-300">
            ...
            <button
              ...
              className={`text-sm font-semibold px-4 py-2 rounded-xl border transition-all duration-200 ${
                active === cat
                  ...
                  : "bg-white dark:bg-white/5 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/10 hover:border-indigo-400 dark:hover:border-blue-400 hover:text-indigo-600 dark:hover:text-blue-400"
              }`}
            >
              {cat}
              {cat !== "همه" && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
          {filtered.map((event) => {
            ...
            return (
              <article
                key={event.id}
                className="group flex flex-col bg-white dark:bg-white/[0.03] border border-slate-100 dark:border-white/8 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:shadow-slate-200/60 dark:hover:shadow-black/30 hover:-translate-y-1 transition-all duration-300"
              >
                {/* تصویر placeholder */}
                ...
                <div className="flex flex-col flex-1 p-5 gap-4">
                  <h3 className="text-base font-bold text-slate-900 dark:text-white leading-snug group-hover:text-indigo-600 dark:group-hover:text-blue-400 transition-colors duration-200">
                    {event.title}
                  </h3>
                  ...
                  <div className="flex flex-col gap-2 text-sm text-slate-500 dark:text-slate-400">
                    <div className="flex items-center gap-2">
                      <Calendar className="w-4 h-4 flex-shrink-0 text-indigo-400 dark:text-blue-500" />
                      <span>{event.date}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock className="w-4 h-4 flex-shrink-0 text-indigo-400 dark:text-blue-500" />
                      <span>{event.time}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <MapPin className="w-4 h-4 flex-shrink-0 text-indigo-400 dark:text-blue-500" />
                  ...
                  <div className="mt-auto flex flex-col gap-2">
                    <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                      <span className="flex items-center gap-1">
                        ...
                        {event.registered} نفر ثبت‌نام کردن
                      </span>

...
export default function ProjectsPage() {
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <aside className="w-full lg:w-80 flex flex-col gap-4">
            ...
            <div className="border rounded-2xl overflow-hidden transition-colors" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border)" }}>
              ...
              <div className={`transition-all duration-300 overflow-hidden ${openSection.techs ? "max-h-80 border-t overflow-y-auto" : "max-h-0"}`} style={{ borderColor: "var(--border)" }}>
  ...
  <button
    ...
    style={{
      backgroundColor: activeTech === tech ? "var(--accent)" : "transparent",
      // تغییر این خط از activeCategory به activeTech:
      color: activeTech === tech ? "#ffffff" : "var(--text)" 
    }}
    onMouseEnter={(e) => activeTech !== tech && (e.currentTarget.style.backgroundColor = "var(--social-bg)")}
    onMouseLeave={(e) => activeTech !== tech && (e.currentTarget.style.backgroundColor = "transparent")}
  >
    {tech}
  </button>
))}
          ...
          <main className="flex-1 flex flex-col gap-6">
            ...
            <div className="border rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm transition-colors" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border)" }}>
              ...
              <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                ...
                <div className="relative border rounded-xl px-3 py-2 text-xs font-bold flex items-center gap-1 shadow-sm" style={{ backgroundColor: "var(--bg)", borderColor: "var(--border)", color: "var(--text-h)" }}>
                  ...
                  >
                    <option value="id" className="bg-[var(--card-bg)]">جدیدترین</option>
                    <option value="teamCount" className="bg-[var(--card-bg)]">تعداد اعضای تیم</option>
                    <option value="techCount" className="bg-[var(--card-bg)]">تعداد تکنولوژی‌ها</option>
                  </select>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto border-t sm:border-t-0 pt-3 sm:pt-0" style={{ borderColor: "var(--border)" }}>
                <span className="text-xs font-bold opacity-80" style={{ color: "var(--text)" }}>
                  نمایش نتیجه <span style={{ color: "var(--accent)" }}>{filteredProjects.length}</span> از {PROJECTS.length}
                </span>
            ...
            {filteredProjects.length > 0 ? (
                ...
                {filteredProjects.map((project) => (
                  ...
                  >
                    <div className={`relative overflow-hidden bg-gradient-to-br ${project.gradient} transition-transform duration-300 ${
                      viewMode === "grid" ? "w-full h-36" : "w-full sm:w-44 h-32 rounded-xl flex-shrink-0"
              ...
              <div className="text-center py-20 border border-dashed rounded-2xl" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border)" }}>
                ...
                <p className="font-bold text-base" style={{ color: "var(--text-h)" }}>پروژه‌ای با این مشخصات یافت نشد!</p>
              </div>
            )}
